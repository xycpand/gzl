	...
	<head>
		...
		<script type="text/javascript">
			...
			function save(){
				$.ajax({
                   type: "POST",
                   url: '${pageContext.request.contextPath}/zdycd/saveMessage.do',
...
<body>
	<form action="${pageContext.request.contextPath}/zdycd/update.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
		<div id="zhongxin">
		<table width="90%">
			<tr>
				<c:if test="${pd.lx=='fstw'}">
					<td width="20%" align="right">图文类型：</td><td><input type="text" name="twlx" id="twlx" value="${p.twlx }" style="width: 95%" placeholder="这里输入图文事件类型" title="图文事件类型"/></td>
				</c:if>
				<c:if test="${pd.lx=='tzlj'}">
					<td width="20%" align="right">跳转链接：</td><td><input type="text" name="url" id="url" value="${p.url }" style="width: 95%" placeholder="这里输入跳转链接" title="跳转链接"/></td>
				</c:if>
			</tr>
			<tr>
				<td style="text-align: center;" colspan="2">
					...
					<input type="hidden" id="lx" name="lx" value="${pd.lx }">
				</td>
			</tr>
		</table>
		</div>
