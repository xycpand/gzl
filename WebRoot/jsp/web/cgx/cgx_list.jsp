...
<ul class="breadcrumb">
	<li><i class="icon-home"></i> <a>报表管理</a><span class="divider"><i class="icon-angle-right"></i></span></li>
...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="${pageContext.request.contextPath}/cgx/findList.do?isM1=${pdm.isM1 }&isM2=${pdm.isM2 }" method="post" name="userForm" id="userForm">
			<table>
				<tr>
							...
							<tr>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<c:if test="${QX.edit == 1 }">
										<a class='btn btn-mini btn-info' title="生成表单" onclick="scbd('${var.id }');">生成表单</a>
										<a class='btn btn-mini btn-info' title="下发调查表" onclick="scdcb('${var.id }');">生成调查表</a>
...
<!--		<script type="text/javascript" src="js/attention/zDialog/zDialog.js"></script>-->
		...
		//新增
		function add(){
			 art.dialog.open('${pageContext.request.contextPath}/cgx/toadd.do',{
		   	 	title:'新增',
				width:1000,
	    		height:560,
	    		lock: true
		...
		//修改
		function edit(id){
			art.dialog.open('${pageContext.request.contextPath}/cgx/toedit.do?id='+id,{
		   	 	title:'编辑',
				width:1000,
	    		height:560,
	    		lock: true
		...
		function del(id,status){
			if(status=='Y'){
				...
				bootbox.confirm("确定要删除该记录?", function(result) {
					if(result) {
						var url = "${pageContext.request.contextPath}/cgx/remove.do?id="+id;
		...
		function scdcb(id){
			art.dialog.open('${pageContext.request.contextPath}/cgx/findYhxx.do?id='+id,{
		   	 	title:'编辑',
				width:280,
	    		height:370,
	    		lock: true
		...
		function scbd(id){
			art.dialog.open('${pageContext.request.contextPath}/jsp/web/cgx/cgx_lcxx.jsp?id='+id,{
		   	 	title:'选择流程',
				width:320,
	    		height:240,
	    		lock: true
		...
		function scbd_bak(id){
			bootbox.confirm("确定生成表单吗?", function(result) {
				if(result) {
					$.ajax({
	                   type: "POST",
	                   url: '${pageContext.request.contextPath}/cgx/saveBdxx.do?id='+id,
	                   data: '',
