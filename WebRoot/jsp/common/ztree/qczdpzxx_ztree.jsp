...
<script type="text/javascript">
	...
	$(document).ready(function (){ 
		var treeNodes = '';
			$.ajax({
			    url : "${pageContext.request.contextPath}/qcgl/qczdpzxx/findJson.do?qcjbxxid=${pd.qcjbxxid}&lx=system",
			    data : {},
			    type : 'GET',
			    dataType : 'json',
			    success : function(json) {
				   treeNodes += "[{id:'0',pId:'100',name:'配置大全',lx:'',dj:'gjd'},";
	...
	function expandFirst(){
		...
		$("#m_lx").val(nodes[0].lx);
		$("#m_dj").val(nodes[0].dj);
		show(nodes[0].id,nodes[0].name,nodes[0].dj);
	...
	function zTreeOnClick(event, treeId, treeNode) {
		show(treeNode.id,treeNode.name,treeNode.dj);
		$("#m_id").val(treeNode.id);
		$("#m_name").val(treeNode.name);
		$("#m_lx").val(treeNode.lx);
		$("#m_dj").val(treeNode.dj);
	};
	
	//显示
	function show(id,name,dj){
		$.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/qcgl/qczdpzxx/findJson.do?pid="+id+"&qcjbxxid=${pd.qcjbxxid}&lx=system",
            dataType: "json",
            	...
            	var tt ='';
            		$.each(result,function(i,n){
            			...
            			tt+='<td style="width:120px;">';
            				...
            				if(n.lx!='system'){
            					tt+='<a class="btn btn-mini btn-danger" title="删除" onclick="del(\''+n.id+'\',\''+n.val_id+'\');"><i class="icon-trash"></i></a>';
            				}
            			tt+='</td>';
            			tt+='</tr>';
            		});
            		...
            		if(dj=='gjd'){
            			var th = '<tr><th>序号</th><th>名称(key)</th><th>排序</th><th>修改时间</th><th>操作</th></tr>';
            			$("#mtd").html(th);
            		}else if(dj=='yjjd'){
            			var th = '<tr><th>序号</th><th>名称(key)</th><th>值(value)</th><th>排序</th><th>修改时间</th><th>操作</th></tr>';
            			$("#mtd").html(th);
            		}
            		$("#mtb").html(tt);
            },
