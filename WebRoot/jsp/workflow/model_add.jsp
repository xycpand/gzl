	...
	<head>
		...
		<script type="text/javascript">
			...
			function save(){
				...
				if(namenum==0){
					art.dialog.alert("请输入流程模型名称。");
				}else if(keynum==0){
					art.dialog.alert("请选择流程模型Key。");
				}else{
					$("#cdForm").submit();
				}
			}
...
<c:if test="${pd.label!='b'}">
	<form action="${pageContext.request.contextPath}/workflow/addModel.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
		<div id="zhongxin">
		<table width="90%">
			<tr>
				<td width="20%" align="right">流程模型名称：</td><td><input type="text" name="name" id="name" style="width: 95%" placeholder="这里输入流程名称" title="流程名称"/></td>
			</tr>
			<tr>
				<td width="20%" align="right">流程模型Key：</td><td><input type="text" name="key" id="key" style="width: 95%" placeholder="这里输入流程Key" title="流程Key"/></td>
			</tr>
			<tr>
				<td width="20%" align="right">描述：</td>
				<td>
					<textarea rows="8" cols="8" name="description" id="description" style="width: 95%"></textarea>
				</td>
			</tr>
			<tr>
