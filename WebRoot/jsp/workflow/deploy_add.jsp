	...
	<head>
		...
		<script type="text/javascript">
			...
			function save(){
				...
				var file = $("#file").val();
				var filenamenum = getByteLen(filename);
				var filenum = getByteLen(file);
				
				if(filenamenum==0){
					art.dialog.alert("请输入流程名称。");
				}else if(filenum==0){
					art.dialog.alert("请选择流程文件。");
				}else{
					$("#cdForm").submit();
				}
			}
...
<c:if test="${pd.label!='b'}">
	<form action="${pageContext.request.contextPath}/workflow/saveNewDeploye.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
		<div id="zhongxin">
		<table width="90%">
			<tr>
				<td width="20%" align="right">流程名称：</td><td><input type="text" name="filename" id="filename" style="width: 95%" placeholder="这里输入流程名称" title="流程名称"/></td>
			</tr>
			<tr>
				<td width="30%" align="right">流程文件:</td><td>
				<input type="file" style="width: 95%" name="file" id="file">
				</td>
			</tr>
			<tr>
