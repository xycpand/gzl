...
<html lang="en">
	<head>
		...
		<script type="text/javascript">
			...
			function save(){
				var yhid = $("#yhid").val();
				var yhidnum = getByteLen(name);
				
				if(yhidnum==0){
					art.dialog.alert("请选择角色用户。");
				}else{
					$("#cdForm").submit();
				}
			}
			...
			function findUser(){
				$.ajax({
					url : '${pageContext.request.contextPath}/wfuser/findUser.do',
					data : {
					},
					dataType : 'json',
...
<body>
	<c:if test="${pd.flag!='add'}">
		<form action="${pageContext.request.contextPath}/wfuser/save.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
			<div id="zhongxin">
			<table width="90%">
				<tr>
					<td width="20%" align="right">上级领导名称：</td><td><input type="text" readonly value="${pd.pdqmc }" style="width: 95%" placeholder="父节点" title="父节点"/></td>
				</tr>
				<tr>
					<td width="20%" align="right">角色用户名称：</td>
					<td>
						<select name="yhid" id="yhid" style="width:95%">
						</select>
					</td>
				</tr>
				<tr>
