			...
			function findUser(){
				$.ajax({
					...
					success : function(data) {
						var tt = '<option value="" disabled="true">请选择</option>';
						$.each(data, function(i, n) {    
							if(yhid==n.id){
								tt+='<option value="'+n.id+'" selected="true">'+n.name+'</option>' 
							}else{
...
<body>
	<c:if test="${pd.flag!='add'}">
		<form action="${pageContext.request.contextPath}/dqgl/update.do" name="cdForm" id="cdForm" method="post" enctype="multipart/form-data">
			...
			<table width="90%">
				<tr>
					<td width="20%" align="right">上级领导名称：</td><td><input type="text" readonly value="${pd.pname }" style="width: 95%" placeholder="父节点" title="父节点"/></td>
				</tr>
				<tr>
